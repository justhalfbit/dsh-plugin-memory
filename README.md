...
`memory_save`：用户纠错、带理由的决策、来之不易的教训、明确的用户偏好、稳定的项目事实。拿不准就不存；
保守档还额外提示"多数轮次什么都不记，同一轮的近重复合并成一条"；
...
（默认 64000 字符预算，按需分摊、超出才截断保最新），专题文件每个一行索引（名称 + 大小 + 日期 + 首行摘要），
...
| `injectBudgetChars` | `64000` | 整个注入提示的字符预算（含 ~1K 固定指令开头）；上限而非下限，小记忆只花实际长度。约占 1M token 窗口的 1.6%，满载 200 条时可显示约 150 条 |
...
| 加载 | MEMORY.md 前 200 行 | 主文件预算内全量（默认 64000 字符） |
