 ...
 * durable log; a mid-session change appends a fresh reminder at the tail
 * instead of rewriting history. On process restart + resume the last injected
 * digest is recovered from the session log itself.
 *
 * @module dsh-plugin-memory/inject
 */

import { randomUUID } from 'node:crypto'
import { CATEGORIES, sha1 } from './store.js'
...
function escapeReminder(text) {
  return text.replaceAll(REMINDER_CLOSE, '<\\/system-reminder>')
}
...
export function renderMemoryReminder(parsed, { budgetChars = 4000, topics = [], proactivity = 'conservative' } = {}) {
  ...
  if (topics.length > 0) {
    const lines = topics.slice(0, 24).map((topic) =>
...
export function registerInjection(ctx, store, getConfig) {
  /** @type {WeakMap<object, string>} last injected digest per live session. */
  const lastDigest = new WeakMap()
  /** @type {WeakSet<object>} sessions whose log was scanned for a prior injection. */
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
    ...
    try {
      ...
      lastDigest.set(agent.session, digest)
