...
export const PROACTIVITY_RULES = {
  /**
   * High bar, silence by default, and the only level that states a cadence.
   * Deliberately does NOT re-list the categories: the rule below is the single
   * source of truth, and a copy here is exactly what drifted out of sync with
   * it before (this comment named three of them while `save` carried four).
   */
  conservative: {
    save:
      '- memory_save one entry when you learn something durable and non-obvious: a correction from the user, ' +
      'a decision made with its reason, a hard-won lesson, a stated user preference, or a stable ' +
      'project fact. NEVER save ' +
      'session-specific trivia, restatements of the current task, or casual conversation — when in doubt, do not save.',
    // Pacing, and ONLY for a memory that already has entries: this line must
    // never reach renderBootstrapReminder. That reminder already carries three
    // discouragements, and a fourth would talk a fresh project out of ever
    // recording its first entry — the deadlock c4eb261 exists to break. Kept
    // out of `save` (shared by both paths) and out of `intro` (shared by all
    // three levels, where it would wrongly brake eager).
    cadence:
      '- Most turns record nothing, and that is expected; merge near-duplicates from one turn into a single entry.',
    topics:
 ...
 */
export const DEFAULT_BUDGET_CHARS = 64000
...
/**
 * Exact rendered length of `${heading}\n${lines.join('\n')}`. Counting a
 * newline per line instead overstates every block by one, which at the budget
 * boundary truncated content that actually fit.
 */
function blockLength(heading, lines) {
  const body = lines.reduce((sum, line) => sum + line.length + 1, 0)
  return heading.length + 1 + Math.max(0, body - 1)
}
...
function renderBlock({ heading, lines, grant, keepNewest, omissionLine }) {
  if (blockLength(heading, lines) <= grant) return `${heading}\n${lines.join('\n')}`
...
/**
 * The empty-memory reminder: maintenance rules only, no entry or topic block.
 *
 * Do not collapse this back into `return undefined`: the save rules ship ONLY
 * inside this reminder and `autoDistill` is off by default, so a memory that
 * injects nothing while empty never records a first entry and stays empty for
 * the life of the project. A fixed cost, deliberately outside the character
 * budget — a budget small enough to trim it restores that deadlock. Reports
 * "no entries" rather than "empty" because `topicIndexInInject` can hide topic
 * files that do exist.
 */
function renderBootstrapReminder(rules) {
  const body =
    'Long-term project memory for this working directory (maintained by dsh-plugin-memory; the user ' +
    'can edit the underlying Markdown files) has no entries yet, so none are listed here. Maintain ' +
    'it yourself as you work:\n' +
    `${rules.save}\n` +
    `${rules.topics}\n` +
    '- Do not save anything merely to fill an empty memory. The bar above is the whole test, and a ' +
    'working directory with nothing durable to record yet is supposed to have an empty memory.'
 ...
 *
 * @returns the full reminder text. Never undefined: with no entries and no
 *   topics it degrades to {@link renderBootstrapReminder}, which carries the
 *   maintenance rules that a fresh project needs to record its first entry.
 */
export function renderMemoryReminder(parsed, { budgetChars = DEFAULT_BUDGET_CHARS, topics = [], proactivity = 'conservative' } = {}) {
  ...
  if (populated.length === 0 && indexed.length === 0) return renderBootstrapReminder(rules)
  ...
  const intro =
    ...
    `${rules.save}\n` +
    // Only levels that define a pacing line contribute one; the others render
    // exactly as before.
    (rules.cadence === undefined ? '' : `${rules.cadence}\n`) +
  ...
  const needs = candidates.map((c) => blockLength(c.heading, c.lines))
