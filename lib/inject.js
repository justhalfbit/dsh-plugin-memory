 ...
 *
 * Injection is limited to a turn's FIRST step, which bounds the reminder to at
 * most one copy per turn: the agent maintains this memory itself, so without
 * that gate every memory_save would trigger another full re-injection.
 *
...
/** Share of the variable budget the topic index may claim when entries exist too. */
const TOPIC_INDEX_SHARE = 0.3
 ...
 *
 * `budgetChars` governs the WHOLE reminder. The instruction preamble is fixed
 * and mandatory, so it is paid FIRST and the entries and topic index share
 * what remains — rather than every section quietly claiming a full budget
 * slice on top of an unbudgeted preamble and an unbudgeted topic index.
 *
 * @returns the full reminder text, or undefined when there is nothing to say.
 */
export function renderMemoryReminder(parsed, { budgetChars = 4000, topics = [], proactivity = 'conservative' } = {}) {
  ...
  // Everything that is not an entry or index line still costs characters, so
  // the tags, the blank line after the preamble, and the blank line between
  // blocks are all charged before the content budget is divided.
  ...
  const topicBudget = indexed.length === 0
    ? 0
    : populated.length === 0 ? available : Math.floor(available * TOPIC_INDEX_SHARE)
  const entryBudget = available - topicBudget

  // No artificial per-section floor: the render loop below already guarantees
  // at least one entry per populated section, so a floor could only push the
  // total back over the budget it is supposed to enforce.
  const blocks = []
  const sectionBudget = populated.length === 0 ? 0 : Math.floor(entryBudget / populated.length)
  for (const category of populated) {
    const entries = parsed.sections.get(category.key).entries
    const heading = `## ${category.heading}`
    const lines = []
    // The heading, and the omission line this block may still need, are part
    // of the block's own budget rather than silent extras on top of it.
    let used = heading.length + 1
    const room = entries.length > 1 ? sectionBudget - OMISSION_RESERVE : sectionBudget
    let omitted = 0
    for (let index = entries.length - 1; index >= 0; index -= 1) {
      const entry = entries[index]
      const line = `- [${entry.id}] ${entry.text}`
      if (used + line.length + 1 > room && lines.length > 0) {
        omitted = index + 1
        break
      }
      lines.unshift(line)
    ...
    if (omitted > 0) lines.unshift(`- (… ${omitted} older ${category.heading.toLowerCase()} omitted; use memory_list to read them)`)
    blocks.push(`${heading}\n${lines.join('\n')}`)
  }
  if (indexed.length > 0) {
    const lines = []
    let used = TOPIC_HEADING.length + 1
    const room = indexed.length > 1 ? topicBudget - OMISSION_RESERVE : topicBudget
    let omitted = 0
    for (let index = 0; index < indexed.length; index += 1) {
      const topic = indexed[index]
      const line = `- ${topic.name} (${formatBytes(topic.bytes)}, ${topic.date})${topic.summary.length > 0 ? `: ${topic.summary}` : ''}`
      if (used + line.length + 1 > room && lines.length > 0) {
        omitted = indexed.length - index
        break
      }
      lines.push(line)
      used += line.length + 1
    }
    if (omitted > 0) lines.push(`- (… ${omitted} more topic file(s); use memory_list to see them)`)
    blocks.push(`${TOPIC_HEADING}\n${lines.join('\n')}`)
  }
...
export function registerInjection(ctx, store, getConfig) {
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, step, signal }, next) => {
    ...
    try {
      ...
      // Only the FIRST step of a turn injects. This memory is maintained by the
      // conversation agent itself, so the injected source changes every time it
      // calls memory_save — re-rendering mid-turn would append a fresh full
      // reminder after each save (quadratic context for N saves in one turn),
      // to tell the writer what it just wrote. Changes made during a turn ride
      // the next turn's first step.
