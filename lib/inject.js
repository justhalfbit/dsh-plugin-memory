...
export function renderMemoryReminder(parsed, { budgetChars = 4000, topics = [], proactivity = 'conservative' } = {}) {
  ...
  if (populated.length === 0 && topics.length === 0) return undefined
  const sectionBudget = Math.max(200, Math.floor(budgetChars / Math.max(1, populated.length)))
  const blocks = []
  for (const category of populated) {
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
      ...
      if (used + line.length + 1 > sectionBudget && lines.length > 0) {
        omitted = index + 1
        break
      }
      lines.unshift(line)
    ...
    blocks.push(`## ${category.heading}\n${lines.join('\n')}`)
  }
  if (topics.length > 0) {
    const lines = topics.slice(0, MAX_TOPIC_FILES).map((topic) =>
      `- ${topic.name} (${formatBytes(topic.bytes)}, ${topic.date})${topic.summary.length > 0 ? `: ${topic.summary}` : ''}`)
    blocks.push(`## Topic files (NOT loaded — call memory_read with the topic name when one becomes relevant)\n${lines.join('\n')}`)
  }
  ...
  const intro =
    ...
    `${rules.save}\n` +
    '- memory_forget entries you discover to be wrong or obsolete.\n' +
    rules.topics +
    (topics.length > 0
