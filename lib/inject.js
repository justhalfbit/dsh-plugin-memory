...
export function renderMemoryReminder(parsed, { budgetChars = 4000, topics = [], proactivity = 'conservative' } = {}) {
  ...
  // No per-section floor: the loop already keeps one entry per section, so a
  // floor could only push the total back over the budget it should enforce.
  const blocks = []
  ...
  for (const category of populated) {
    ...
    const lines = []
    // The heading and any omission line come out of the block's own budget.
