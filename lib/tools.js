...
/**
 * The notice every entry-mutating tool appends when the per-category cap
 * destroyed something. Both mutating tools can trigger it: a save pushes a
 * full category over, and a forget on a category ALREADY over its cap (after
 * the limit was lowered, or the file hand-edited) prunes the rest of the
 * excess with it.
 */
function evictionNotice(evicted) {
  return `\nWARNING: a category was over its entry cap, so this call PERMANENTLY DELETED ` +
    `${evicted.length} older entr${evicted.length === 1 ? 'y' : 'ies'} to bring it back within ` +
    `the limit. The deleted text follows — tell the user, and if any of it still matters, ` +
    `preserve it with memory_write_topic rather than re-saving it here (a re-save would just ` +
    `evict the next entry):\n${evicted.join('\n')}`
}

/** Formatted lines for whatever an applyOps result destroyed, in prune order. */
function evictedLines(result) {
  return (result?.pruned ?? []).map((entry) => `[${entry.id}] ${entry.text}`)
}
...
export function registerMemoryTools(ctx, store, getConfig) {
  ctx.tools.register({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          status: { type: 'string' },
          // One formatted line per entry the cap destroyed, like the flat
          // string lists memory_list/memory_search return. Absent entirely
          // when nothing was destroyed, so an ordinary save keeps its shape.
          evicted: { type: 'array', items: { type: 'string' } },
        },
        required: ['id', 'category', 'status'],
      },
      render(args, value) {
        const head = `Memory ${value.status}: [${value.id}] (${value.category})`
        const evicted = value.evicted ?? []
        return [{ type: 'text', text: evicted.length === 0 ? head : head + evictionNotice(evicted) }]
      },
    },
    async execute(args, exec) {
      ...
      const evicted = evictedLines(result)
      return {
  ...
  ctx.tools.register({
    ...
    output: {
      ...
      render(args, value) {
        const head = value.removed ? `Forgot: ${value.text}` : 'No memory entry found with that id.'
        const evicted = value.evicted ?? []
        return [{ type: 'text', text: evicted.length === 0 ? head : head + evictionNotice(evicted) }]
      },
    },
