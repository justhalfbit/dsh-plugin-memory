/**
 * dsh-plugin-memory — the four model-facing memory tools.
 *
...
import { CATEGORY_KEYS, applyOps, countEntries, findEntry, sanitizeTopicName, searchEntries } from './store.js'
...
export function registerMemoryTools(ctx, store, getConfig) {
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      const saved = result.applied.some((line) => line.startsWith('+'))
      const idMatch = /\[([a-z]-[0-9a-f]{3,8})\]/u.exec([...result.applied, ...result.skipped].join('\n'))
      return { id: idMatch?.[1] ?? 'unknown', category, status: saved ? 'saved' : 'duplicate' }
    },
  })
  ...
  ctx.tools.register({
    ...
    parameters: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Entry id, e.g. "f-1a2b3".' },
      },
      required: ['id'],
    },
    output: {
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      const content = typeof args?.content === 'string' ? args.content.trim() : ''
      if (content.length === 0) throw new Error('content is required')
      if (content.length > 16000) throw new Error('content too long for one write: keep it under 16000 characters and append in parts')
