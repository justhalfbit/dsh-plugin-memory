  ...
  ctx.tools.register({
    ...
    output: {
      ...
      render(args, value) {
        ...
        if (evicted.length === 0) return [{ type: 'text', text: head }]
        return [{
          type: 'text',
          text: `${head}\nWARNING: the "${value.category}" category was full, so saving this ` +
            `PERMANENTLY DELETED ${evicted.length} older entr${evicted.length === 1 ? 'y' : 'ies'} ` +
            `to make room. The deleted text follows — tell the user, and if any of it still ` +
            `matters, preserve it with memory_write_topic rather than re-saving it here (a ` +
            `re-save would just evict the next entry):\n${evicted.join('\n')}`,
        }]
      },
    },
    async execute(args, exec) {
      ...
      const evicted = (result.pruned ?? []).map((entry) => `[${entry.id}] ${entry.text}`)
      return {
