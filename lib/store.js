 ...
 *
 * `protectManual` refuses destructive ops against `manual` entries. The rule
 * is about the ACTOR, not the label: a `manual` entry was written through a
 * visible tool call the user could see and object to, so the silent background
 * distiller — which reads a narrow text-only window and produces no tool card —
 * may not delete or rewrite one. Note that `update` is destructive too (it
 * removes then re-adds), so both ops are covered, and a blocked `update` is
 * dropped outright rather than degraded into an `add` that would leave a
 * contradictory pair behind. The visible memory_forget tool passes this off.
 *
