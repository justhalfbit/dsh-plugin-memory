...
/**
 * Stable content-derived entry id: `<category prefix>-<5 hex chars>`.
...
export function entryId(categoryKey, text) {
  ...
  return `${category.prefix}-${sha1(normalizeText(text)).slice(0, 5)}`
}
 ...
 *
 * @returns `{ changed, applied, skipped }` where applied/skipped are
...
export function applyOps(parsed, ops, { date, maxPerCategory = 50 } = {}) {
  ...
  for (const op of ops) {
    if (op.op === 'add') {
      ...
      addEntry(op.category ?? found?.categoryKey ?? 'facts', op.text ?? '', op.source ?? 'auto')
    } else {
...
/** Hard cap on one topic file's character length. */
...
export class MemoryStore {
   ...
   * sessions-style name on first access (rename; on failure — e.g. a
   * cross-process race — keep using the legacy directory).
   */
  async projectDir(cwd) {
    ...
    this.projectDirs.set(key, resolved)
    return resolved
  }
  ...
  /**
   * Read one topic file in full (bounded).
   * @returns `{ path, text }`, or undefined when the topic does not exist.
   */
  async readTopic(cwd, topic, maxChars = 40000) {
      ...
      try {
        ...
        try {
          ...
          if (current.length >= MAX_TOPIC_FILES) {
            throw new Error(`topic file limit reached (${MAX_TOPIC_FILES}); merge or delete existing topics first`)
          }
        }
        ...
        if (next.length > MAX_TOPIC_CHARS) {
          throw new Error(`topic file would exceed ${MAX_TOPIC_CHARS} characters; use mode "replace" or split into another topic`)
        }
        const temp = `${path}.tmp-${process.pid}-${Math.random().toString(36).slice(2, 8)}`
        await writeFile(temp, next, 'utf8')
        await rename(temp, path)
