...
/**
 * `YYYY-MM-DD` for `when` in LOCAL time. `toISOString()` is UTC, so east of
 * Greenwich it stamps anything saved before the UTC day rolls over with
 * yesterday's date — an entry written at 01:00 in UTC+8 was dated a day early.
 */
export function localDay(when = new Date()) {
...
export function parseMemories(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    // An id with no text is not a usable entry, but dropping it would silently
    // delete a line from a hand-edited file; keep it like any other unparsed one.
 ...
 *
 * `protectManual` refuses `forget`/`update` against `manual` entries — the rule
 * is about the ACTOR, not the label: a silent background pass may not undo what
 * a visible tool call recorded. `update` counts because it removes then re-adds,
 * and a blocked one is dropped rather than degraded into a contradictory `add`.
 *
 * @returns `{ changed, applied, skipped, ids, pruned }` — applied/skipped are
 *   human-readable audit lines; `ids.added`/`ids.duplicates` are the
 *   structured entry ids for programmatic consumers (memory_save). `pruned`
 *   carries the entries the cap destroyed, TEXT included: the id alone would
 *   be useless to report, since the entry it names is already gone from the
 *   file and no later `memory_list` can show what was lost.
 */
export function applyOps(parsed, ops, { date, maxPerCategory = 50, protectManual = false } = {}) {
  const today = date ?? localDay()
  const applied = []
  const skipped = []
  ...
  /** Whether `found` is a manual entry this caller is not allowed to destroy. */
  const isProtected = (found) => protectManual && found?.entry.source === 'manual'
  ...
  for (const op of ops) {
    if (op.op === 'add') {
      ...
      if (isProtected(found)) {
        skipped.push(`protected manual entry [${op.id}] (a silent background pass may not remove it)`)
        continue
      }
      ...
      const found = typeof op.id === 'string' ? findEntry(parsed, op.id) : undefined
      if (isProtected(found)) {
        skipped.push(`protected manual entry [${op.id}] (a silent background pass may not rewrite it)`)
        continue
      }
  ...
  for (const section of parsed.sections.values()) {
    while (section.entries.length > maxPerCategory) {
      ...
      pruned.push({ id: dropped.id, text: dropped.text })
      changed = true
    }
  }

  return { changed, applied, skipped, ids, pruned }
}
...
/**
 * The ONE topic size limit: a file, a read, and a single write are all bounded
 * by it, so anything the store accepted reads back whole and can be rewritten
 * whole. A smaller read cap would hand back a truncated body that a later
 * `replace` would save over the complete file.
 */
...
export class MemoryStore {
    ...
    for (const file of files.sort()) {
      ...
      try {
        ...
        topics.push({
          name: file.slice(0, -3),
          bytes: info.size,
          date: localDay(new Date(info.mtimeMs)),
   ...
   * every other topic mutation. Without it the per-project cap is a dead end.
   ...
   *
   * EVERY retry path — contended, stolen, failed steal, vanished mid-inspection
   * — shares one attempt bail and one backoff, so an unremovable lock (foreign
   * owner, read-only mount) degrades into "proceed unlocked" rather than
   * spinning without a timeout.
   */
    ...
    for (let attempt = 0; ; attempt += 1) {
      try {
        ...
        try {
          ...
          // A failed stat or steal is not fatal: fall through to the backoff.
        }
