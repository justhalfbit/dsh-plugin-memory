...
const ConfigSchema = Schema.object({
  ...
  injectBudgetChars: Schema.number().min(2500).default(DEFAULTS.injectBudgetChars)
    .description('Character budget for the WHOLE injected memory reminder: its fixed instruction preamble (~1KB) is paid first, and the entries plus topic index share the remainder. One entry per populated section is always kept, so with unusually long entries the result can still exceed a near-minimum budget by a few hundred characters.'),
