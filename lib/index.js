...
const DEFAULTS = {
  ...
  autoDistill: false,
  memoryDir: '',
  // Sized for a modern context window: ~0.4% of a 1M-token one, showing ~36
  // entries, where the old 4000 could only ever show four. Owned by the
  // renderer so the schema default and its own fallback stay one value.
...
const ConfigSchema = Schema.object({
  ...
  // gracefully, whereas a stricter schema bound would fail the whole namespace
  // registration — costing the user every memory setting and the settings card
  // itself — over a value that actually works.
  injectBudgetChars: Schema.number().min(500).default(DEFAULTS.injectBudgetChars)
    .description('Character budget for the WHOLE injected memory reminder: its fixed instruction preamble (~1KB) is paid first, and the entries plus topic index share the remainder. One entry per populated section is always kept, so below roughly 2500 the result exceeds the budget rather than dropping content.'),
