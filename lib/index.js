 ...
 *
 * 1. Silent two-phase distillation on every completed `turn/end` (zero-cost
 *    pre-checks, then one direct `ctx.llm.stream()` call — no tool cards, no
 *    conversation output) into `~/.dsh/memory/projects/<slug>/memories.md`.
 * 2. `agent/pre-step` injection of that Markdown as a digest-deduplicated
 *    `<system-reminder>` (KV-cache friendly).
 * 3. Four global tools: memory_save / memory_search / memory_list /
 *    memory_forget.
