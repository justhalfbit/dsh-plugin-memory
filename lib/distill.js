 ...
 *
 * Because this pass is silent, it is deliberately weaker than the in-conversation
 * tools: it may add entries and curate its own `auto` ones, but `protectManual`
 * refuses any op that would delete or rewrite an entry written through a visible
 * tool call. It also sees far less — a text-only window with no tool calls or
 * results — so it is the wrong actor to be second-guessing that record.
 *
 ...
 * Each line carries its provenance: `manual` entries are off-limits to this
 * pass (enforced in applyOps, not merely requested here), so showing the
 * source stops the model from spending its small op budget on revisions that
  ...
  const run = async (session, turn) => {
    ...
    try {
      ...
      const result = await store.mutate(cwd, (parsed) =>
        // protectManual: this pass is silent and produces no tool card, so it
        // may add freely and curate its own `auto` entries, but it may not
        // delete or rewrite what a visible tool call recorded.
